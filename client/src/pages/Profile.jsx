      ...
      {userListings && userListings.length > 0 &&
        ...
        <div className='flex flex-col gap-4'>
          ...
          {userListings.map((listing) =>
            <div key={listing._id} className='border gap-4 rounded-lg p-3 flex justify-between 
              items-center'>
              <Link to={`/listing.imageUrls[0]`}>
                <img src={listing.imageUrls[0]}
                  className='h-16 w-16 object-contain ' alt='listing cover' />

              </Link>
              <Link className='flex-1 text-slate-700 font-semibold 
             hover:underline truncate gap-4' to={`/listing/${listing._id}`}>
                <p>{listing.name}</p>
              </Link>
